...
class IVACApp(ctk.CTk):
    ...
    def _build_main_ui(self):
        ...
        ctk.CTkLabel(
            header_inner,
            text=f"🔑 {self.license_info.plan} | {self.license_info.days_remaining}d",
    ...
    def _build_home_tab(self):
        ...
        self.server_info_label = ctk.CTkLabel(
            server_details,
            text="Port: 5000  |  SMS Endpoint: POST http://<PC_IP>:5000/api/sms",
    ...
    def _open_add_profile_dialog(self):
        ...
        ctk.CTkLabel(
            dialog, 
            text="💡 টিপস: সঠিক Chrome Profile Name জানতে আপনার ক্রোম\nব্রাউজারের এড্রেসবারে chrome://version/ লিখে এন্টার দিন এবং\n'Profile Path'-এর শেষের ফোল্ডারের নামটি এখানে দিন।",
            font=ctk.CTkFont(size=11),
            text_color="#9ca3af",
            justify="left"
    ...
    def _build_settings_tab(self):
        ...
        # SIM Mapping
        sim_card = ctk.CTkFrame(tab, fg_color="#112240", corner_radius=10)
        sim_card.pack(fill="x", padx=5, pady=(5, 5))
        
        ctk.CTkLabel(
            sim_card, text="📱 SIM Mapping (ফোন নম্বর ম্যাপিং)",
        ...
        ctk.CTkLabel(
            sim_card,
            text="SIM Mapping পরিবর্তন করতে sim_mapping.json ফাইলটি এডিট করুন",
            font=ctk.CTkFont(size=11),
            text_color="#8892b0"
        ).pack(anchor="w", padx=15, pady=(0, 10))
        
        # Dashboard Link
        dash_card = ctk.CTkFrame(tab, fg_color="#112240", corner_radius=10)
        dash_card.pack(fill="x", padx=5, pady=5)
        
        ctk.CTkLabel(
            dash_card, text="📊 Web Dashboard",
            font=ctk.CTkFont(size=13, weight="bold"),
            text_color="#ccd6f6"
        ).pack(anchor="w", padx=15, pady=(10, 5))
        
        ctk.CTkButton(
            dash_card, text="🌐 Dashboard খুলুন (Browser)",
            font=ctk.CTkFont(size=12),
            height=35, width=250,
            fg_color="#233554", hover_color="#059669",
    ...
    def _build_license_tab(self):
        ...
        rows = [
            ...
            ("Remaining:", f"{info.days_remaining} দিন" if info else "N/A"),
        ...
        ctk.CTkButton(
            btn_frame, text="🔄 নতুন লাইসেন্স কোড দিন",
            font=ctk.CTkFont(size=12, weight="bold"),
            height=38,
            fg_color="#233554", hover_color="#059669",
            command=self._reactivate_license
        ).pack(fill="x")
    
    def _reactivate_license(self):
        info = check_license()
        info.status = LicenseStatus.NOT_ACTIVATED
        LicenseActivationWindow(self, info, self._on_reactivated)
    
    def _on_reactivated(self, info):
        self.license_info = info
        self._build_main_ui()
    ...
    def _export_extension(self):
        ...
        ext_dest = os.path.join(desktop_path, "IVAC_Chrome_Extension")
        
        try:
            if os.path.exists(ext_dest):
                shutil.rmtree(ext_dest)
            shutil.copytree(ext_src, ext_dest)
            messagebox.showinfo("Success", f"Extension exported successfully to Desktop:\\n{ext_dest}")
        except Exception as e:
